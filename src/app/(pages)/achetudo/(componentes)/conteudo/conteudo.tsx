"use client";

export default function Conteudo() {
  return (
    <>
      <div id="vantagens" className="mt-0"></div>
      <div className="static m-0 h-auto w-auto justify-center pb-25">
        <div className="m-auto p-6 text-center">
          <span className="text-2xl font-bold text-blue-950 text-shadow-lg">
            Quais as vantagens em ter um site ?
          </span>
        </div>
        <div className="m-auto mb-5 h-55 w-55 justify-center rounded-3xl border-1 border-white p-12 text-center backdrop-blur-sm">
          <span>
            Apareça nas pesquisas do google e localização google maps.
          </span>
        </div>
        <div className="m-auto mb-5 h-55 w-55 justify-center rounded-3xl border-1 border-white p-12 text-center backdrop-blur-sm">
          <span>Contato do cliente diretamente por email ou Whatsapp.</span>
        </div>
        <div className="m-auto mb-5 h-55 w-55 justify-center rounded-3xl border-1 border-white p-12 text-center backdrop-blur-sm">
          <span>
            Sua loja e produtos viram um cartão de visita virtual na internet.
          </span>
        </div>
        <div className="m-auto mb-5 h-55 w-55 justify-center rounded-3xl border-1 border-white p-12 text-center backdrop-blur-sm">
          <span>
            Mostre sua loja ou produto nas redes sociais usando o site.
          </span>
        </div>
        <div className="z-[-1] m-auto h-50 w-50 bg-[url('https://achetudotiradentes-nextjs-app.s3.sa-east-1.amazonaws.com/pages/4570f0bd-125f-4319-846e-9e263ffa66ee/fundo_chamada_2.png')] bg-size-[100%_100%] bg-center bg-no-repeat"></div>
      </div>
    </>
  );
}
